...
function extractFromCode(code, fileName) {
  const babelOptions = {
    ast: true,
    filename: fileName,
    presets: [
...
export default function extractFromFiles(filePaths, options) {
  const keys = [];
  ...
  filePaths.forEach((filePath) => {
    toScan = toScan.concat(sync(filePath, {}));
  });
  ...
  toScan.forEach((filePath) => {
    ...
    const extractedKeys = extractFromCode(code, fileName, options);
    extractedKeys.forEach((keyObj) => {
      keyObj.file = filePath;
